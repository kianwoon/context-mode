...
export function loadDatabase(): typeof DatabaseConstructor {
  if (!_Database) {
    ...
    if ((globalThis as any).Bun) {
      ...
      // Node.js — try node:sqlite first (built into Node >= 22.5, no native deps).
      // Falls back to better-sqlite3 for older Node versions.
      try {
        ...
        // node:sqlite not available — fall back to better-sqlite3
